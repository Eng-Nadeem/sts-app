...
export const mongoStorage = {
	...
	async getRecentMeters(limit = 5) {
		return await Meter.find().sort({ createdAt: -1 }).limit(limit);
	},
	...
	async getTransactionStats() {
		...
		const totalAmount =
			aggregation.length > 0 ? aggregation[0].totalAmount : 0;
	...
	async getUserDebts(userId: ObjectIdOrString) {
		return await Debt.find({ userId: toObjectId(userId) }).sort({
			dueDate: 1,
		});
	},
