...
const SuccessScreen = () => {
	...
	const estimatedUnits =
		!isDebtPayment && !isMultipleDebtPayment && displayData.amount
			? (parseFloat((displayData.amount || 0).toString()) / 0.45).toFixed(
					1
			  )
			: "0.0";
			...
			<div className="text-center px-4">
				...
				{!isDebtPayment && displayData.token && (
					<div className="bg-gray-50 border border-gray-200 rounded-lg p-4 mb-6 text-left">
						<h3 className="font-semibold mb-2">معلومات الرمز</h3>
						<p className="text-2xl font-mono text-center bg-white border border-gray-300 rounded py-2 mb-2">
							{displayData.token}
						</p>
						<p className="text-sm text-gray-500">
							أدخل هذه الرمز في عدادك المدفوع مقدمًا لإكمال الشحن
						</p>
					</div>
				)}
