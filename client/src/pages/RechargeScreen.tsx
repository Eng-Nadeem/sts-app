...
const RechargeScreen = () => {
  ...
  const onSubmit = async (data: FormData) => {
    try {
      ...
      navigate(`/payment-confirmation?meterNumber=${data.meterNumber}&nickname=${data.nickname || ''}&amount=${data.amount}&paymentMethod=${data.paymentMethod}`);
    } catch (error) {
      toast({
        variant: "destructive",
        title: "Error",
        description: "Failed to save meter information. Please try again."
      });
    }
  };
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                    ...
                    <div className="relative">
                      ...
                      <FormControl>
                        <Input 
                          ...
                          className={`pl-10 ${!validateMeterNumber(field.value) && field.value ? 'border-error' : ''}`}
                          inputMode="numeric"
                        />
                      </FormControl>
                    </div>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>Meter Nickname (Optional)</FormLabel>
                    <div className="relative">
                    ...
                    <div className="grid grid-cols-3 gap-2 mb-2">
                      {PRESET_AMOUNTS.map((amount) => (
                        <Button
                          ...
                          className={presetAmount === amount ? "bg-blue-100 border-primary text-primary" : ""}
                        >
                          ${amount}
                        </Button>
                      ))}
                    ...
                    <div className="relative">
                      ...
                      <FormControl>
                        <Input 
                          ...
                          className={`pl-10 ${!validateAmount(field.value) && field.value ? 'border-error' : ''}`}
                          inputMode="decimal"
                        />
                      </FormControl>
                    </div>
                    ...
                    <FormControl>
                      ...
                      >
                        <div className="bg-gray-50 p-3 rounded-lg border border-gray-200 flex items-center">
                          <RadioGroupItem value="card" id="card-payment" className="mr-3" />
                          <Label 
                        ...
                        <div className="bg-gray-50 p-3 rounded-lg border border-gray-200 flex items-center">
                          <RadioGroupItem value="mobile" id="mobile-money" className="mr-3" />
                          <Label 
