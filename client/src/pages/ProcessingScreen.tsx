	...
	useEffect(() => {
		const processPayment = async () => {
			try {
				...
				navigate(
					`/success?transactionId=${data.id}&meterNumber=${meterNumber}&amount=${amount}&token=${data.token}`
				);
			} catch (error) {
				...
				toast({
					...
					title: "فشل الدفع",
					description:
						"لم نتمكن من معالجة مدفوعتك. يرجى المحاولة مرة أخرى.",
				});
	...
	return (
		<div className="slide-in h-full flex items-center justify-center">
			<div className="text-center px-4">
				...
				<h2 className="text-xl font-semibold mb-2">معالجة الدفع</h2>
				<p className="text-gray-500 mb-8">
					يرجى الانتظار بينما نقوم بمعالجة دفعك...
				</p>

				<Card className="mb-4 mx-auto max-w-xs">
					<CardContent className="p-4">
						<div className="flex justify-between mb-2">
							<span className="text-gray-500">العداد:</span>
				...
				<p className="text-sm text-gray-500">
					قد يستغرق هذا بضع لحظات. يرجى عدم إغلاق هذه الشاشة.
				</p>
			</div>
		</div>
	);
